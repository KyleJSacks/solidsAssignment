...
def scanline_convert(polygons, i, screen, zbuffer, color ):
    ...
    if (b[1] == t[1]):
        ...
        #print('test')
    #print(m, b, t)
    yOffset0 = ceil(b[1]) - b[1]
    yOffset1 = ceil(m[1]) - m[1]
    #print(x0, x1, y)
    dx0 = (t[0] - b[0]) / (t[1] - b[1])
    dz0 = (t[2] - b[2]) / (t[1] - b[1])
    try:
    ...
    try:
        ...
        dx1_1 = t[0] - m[0] 
    try:
        dz1_1 = (t[2] - m[2]) / (t[1] - m[1])
    except:
        dz1_1 = t[2] - m[2]
    x0 = b[0] + (yOffset0 * dx0)
    x1 = b[0] + (yOffset0 * dx1)
    x2 = m[0] + (yOffset1 * dx1_1)
    z0 = b[2] + (yOffset0 * dz0)
    z1 = b[2] + (yOffset0 * dz1)
    z2 = m[2] + (yOffset1 * dz1_1)
    y = ceil(b[1])
    while y < ceil(m[1]):
        draw_line(ceil(x0), y, z0, ceil(x1), y, z1, screen, zbuffer, color)
        x0 += dx0
        x1 += dx1
        y += 1
    
    while y < ceil(t[1]):
        draw_line(ceil(x0), y, z0, ceil(x2), y, z2, screen, zbuffer, color)
        x0 += dx0
        x2 += dx1_1
        y += 1
