...
"""
ARG_COMMANDS = [ 'box', 'sphere', 'torus', 'circle', 'bezier', 'hermite', 'line', 'scale', 'move', 'rotate', 'save', 'polygon' ]
...
def parse_file( fname, edges, polygons, csystems, screen, zbuffer, color ):
    ...
    while c < len(lines):
        ...
        if line == 'push':
            ...
            add_polygon(polygons, float(args[0]), float(args[1]), float(args[2]),
                        float(args[3]), float(args[4]), float(args[5]), float(args[6]), float(args[7]), float(args[8]))
